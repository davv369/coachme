...
import {
  TRAINING_PLAN_IN_PORT,
  TrainingPlanInPort,
} from '@modules/training-plans/application/ports/in/training-plan.in-port';
...
export class TrainingPlanHttpInAdapter {
  ...
  async createTrainingPlan(
    ...
    const plan = await this.trainingPlanInPort.createTrainingPlan({
  ...
  async getTrainingPlanById(
    ...
    const plan = await this.trainingPlanInPort.findTrainingPlanById({ id });
    ...
    // Check permissions: trainer or athlete
    if (
  ...
  async getTrainingPlans(
    ...
    if (jwtPayload?.role === UserRole.TRAINER) {
      // Trener widzi swoje plany
      plans = await this.trainingPlanInPort.findTrainingPlansByTrainer({
        trainerId: jwtPayload.sub,
      });
    } else if (jwtPayload?.role === UserRole.ATHLETE) {
      // Zawodnik widzi swoje plany
      plans = await this.trainingPlanInPort.findTrainingPlansByAthlete({
        athleteId: jwtPayload.sub,
      });
    } else if (athleteId) {
      // Admin can filter by athleteId
      plans = await this.trainingPlanInPort.findTrainingPlansByAthlete({
        athleteId,
      });
    } else {
  ...
  async updateTrainingPlan(
    ...
    const updatedPlan = await this.trainingPlanInPort.updateTrainingPlan({
      id,
      name: dto.name,
      description: dto.description,
      status: dto.status,
  ...
  async addWorkoutToPlan(
    ...
    const plan = await this.trainingPlanInPort.findTrainingPlanById({
      id: trainingPlanId,
    });
    ...
    const workout = await this.trainingPlanInPort.addWorkoutToPlan({
      trainingPlanId,
  ...
  async getWorkoutsByPlan(
    ...
    const workouts = await this.trainingPlanInPort.findWorkoutsByPlan({
      trainingPlanId,
    });
  ...
  async removeWorkoutFromPlan(
    ...
    // Only trainer can remove workouts
    if (
      jwtPayload.role !== UserRole.ADMIN &&
      plan.trainerId !== jwtPayload.sub
    ) {
    ...
    await this.trainingPlanInPort.removeWorkoutFromPlan({
      id: workoutId,
      trainingPlanId,
    });
  }
